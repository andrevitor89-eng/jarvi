...
export function buildWebExtras(_ctx: AgentContext): string {
  return joinNonEmpty([
    ...
    '- PERGUNTA DE PRAZO (OBRIGATÓRIO QUANDO SEM DATA): Se a tarefa foi criada SEM due_date e o usuário NÃO pediu lembrete, faça UMA pergunta curta sugerindo um prazo (due_date) adequado (ex: "Quer definir uma data pra isso?", "Quer marcar a véspera como prazo?"). Pode ficar sem data. Faça só 1 pergunta.',
    '- PERGUNTA DE LEMBRETE (OBRIGATÓRIO QUANDO PEDIRAM AVISO SEM DETALHE): Se o usuário pediu para ser lembrado/avisado e create_task/update_task voltou sem reminders (reminders_count=0), faça UMA pergunta sobre canal e/ou horário do aviso (ex: "Te aviso por WhatsApp em que horário?"). Não pergunte prazo de novo se a data já existe; foque no lembrete. Se a tarefa JÁ foi criada COM data/horário e NÃO pediram lembrete, no máximo 1 pergunta de contexto curta, se for útil.',
